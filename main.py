...
def modeling(df, feature_cols, seed):
    ...
    try:
        ...
        running_times["without_normalization"] = duration
        training_scores["without_normalization"] = training_score
        testing_scores["without_normalization"] = testing_score

        logger.info("-" * 50)
        logger.info(f"Model 1 Train Score(Wighted Percision): {round(training_score,4)}")
        logger.info(f"Model 1 Test Score(Wighted Percision): {round(testing_score,4)}")
        logger.info(f"Model 1 training duration: {duration:.2f} seconds")
        logger.info("-" * 50)
        ...
        model_2, training_score, testing_score = modeling_proces(train_df, test_df, [scaler], "scaled_features",
                                                         num_trees, max_depth, seed)
        duration = time.time() - start_time
        running_times["with_normalization"] = duration
        training_scores["with_normalization"] = training_score
        testing_scores["with_normalization"] = testing_score

        logger.info(f"Model 2 Train Score(Wighted Percision): {round(training_score,4)}")
        logger.info(f"Model 2 Test Score(Wighted Percision): {round(testing_score,4)}")
        logger.info(f"Model 2 training duration: {duration:.2f} seconds")
        logger.info("-" * 50)
        ...
        start_time = time.time()
        scaler_m_3 = StandardScaler(inputCol="features", outputCol="scaled_features", withStd=True, withMean=True)
        pca = PCA(k=pca_k, inputCol="scaled_features", outputCol="pca_features")

        model_3, training_score, testing_score = modeling_proces(train_df, test_df, [scaler_m_3, pca], "pca_features",
                                                         num_trees, max_depth, seed)
        duration = time.time() - start_time
        
        running_times["with_normalization_pca"] = duration
        training_scores["with_normalization"] = training_score
        testing_scores["with_normalization"] = testing_score

        logger.info(f"Model 3 Train Score(Wighted Percision): {round(training_score,4)}")
        logger.info(f"Model 3 Test Score(Wighted Percision): {round(testing_score,4)}")
        logger.info(f"Model 3 training duration: {duration:.2f} seconds")
        logger.info("-" * 50)
